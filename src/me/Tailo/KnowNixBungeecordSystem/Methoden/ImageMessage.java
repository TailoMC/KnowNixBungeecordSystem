...
public class ImageMessage {
    ...
    public ImageMessage(BufferedImage image, int height, char imgChar, String name) {
        ChatColor[][] chatColors = toChatColorArray(image, height);
        lines = toImgMessage(chatColors, imgChar, name);
    }

    public ImageMessage appendText(String... text) {
        for (int y = 0; y < lines.length; y++) {
            if (text.length > y) {
            	TextComponent line = lines[y];
            	line.addExtra(" " + text[y]);
                lines[y] = line;
            }
        }
        return this;
    ...
    private TextComponent[] toImgMessage(ChatColor[][] colors, char imgchar, String name) {
    	TextComponent[] lines = new TextComponent[colors[0].length];
        for (int y = 0; y < colors[0].length; y++) {
            TextComponent line = new TextComponent("");
            line.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("§6" + name + " §abeitreten! §7(/joinme " + name + ")").create()));
            line.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/joinme " + name));
            for (int x = 0; x < colors.length; x++) {
                ChatColor color = colors[x][y];
                line.addExtra("" + ((color != null) ? colors[x][y].toString() + imgchar : TRANSPARENT_CHAR));
            }
            line.addExtra("" + ChatColor.RESET);
            lines[y] = line;
        }
        return lines;
    }
