...
public class COMMAND_tempban extends Command {
	...
	public COMMAND_tempban(String name, main main) {
		super(name);
		this.plugin = main;
	}
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(sender.hasPermission("srmod")) {
			if(args.length == 2) {
				...
				try {
					Ban_methoden.tempban(args[0], null, args[1], sender);
				} catch(Exception e) {
				...
				if(args.length >= 3) {
					...
					try {
						Ban_methoden.tempban(args[0], reason, args[1], sender);
					} catch(Exception e) {
						sender.sendMessage(plugin.prefix + "§cDie Zeit muss eine §eZahl §csein!");
					}
					
				} else {
					sender.sendMessage("§c/tempban [Spielername] [Zeit] [Grund]");
				}
		}
